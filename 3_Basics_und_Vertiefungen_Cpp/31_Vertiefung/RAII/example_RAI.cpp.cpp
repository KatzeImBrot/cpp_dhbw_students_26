...
int main() {
    ...
    // SMART POINTER BEISPIEL (RAII für dynamischen Speicher)
    
    // Ohne RAII: Raw Pointer (gefährlich!)
    int* rawPtr = new int(42);
    std::cout << "Raw Pointer Wert: " << *rawPtr << "\n";
    // Bei Exception hier → Memory Leak!
    delete rawPtr;

    // Mit RAII: unique_ptr (sicher!)
    std::unique_ptr<int> smartPtr = std::make_unique<int>(42);
    std::cout << "Smart Pointer Wert: " << *smartPtr << "\n";
    // Kein delete nötig - automatisch bereinigt beim Verlassen des Scopes!
    // Selbst bei Exception wird der Destruktor aufgerufen.
